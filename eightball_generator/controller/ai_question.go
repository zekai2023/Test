...
func GenerateQuestion(c *gin.Context) {
	...
	for _, topic := range topics {
		...
		category := topicData["category"].(string)          // 获取 category 字段
	...
	aiRequest := AIRequest{
		...
		Messages: []Message{
			...
			{Role: "user", Content: "请根据 " + aiRequestContent + " 生成 3 道选择题，每道题包含问题、四个选项（A/B/C/D）、正确答案、解析，以及领域分类（category）。每道题的格式为一个对象，格式如下：\n{\n  \"category\": \"领域分类\",\n  \"question\": \"问题内容\",\n  \"options\": {\n    \"A\": \"选项A\",\n    \"B\": \"选项B\",\n    \"C\": \"选项C\",\n    \"D\": \"选项D\"\n  },\n  \"correct_answer\": \"正确答案\",\n  \"explanation\": \"解析内容\"\n} \n返回格式必须是JSON数组，每个问题必须包含领域分类字段。"},
		},
	}
...
func Generate_essay_Question(c *gin.Context) {
	// 获取多个知识点，默认用逗号分隔
	topics := c.Query("topics")
	log.Println("Topic received2:", topics)
	...
	aiRequest := AIRequest{
		...
		Messages: []Message{
			{Role: "system", Content: "You are a helpful assistant."},
			{Role: "user", Content: "请根据 " + topics + " 生成 3 道问答题，每道题只包含问题，格式为一个数组，每道题的格式如下：\n{\n  \"question\": \"问题内容\"\n} \n返回格式必须是JSON数组。"},
		},
	}
	// 编码请求数据
	requestData, err := json.Marshal(aiRequest)
